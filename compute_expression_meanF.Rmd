...
author: "Tyler Starr, modified by Allie Greaney"
date: "5/08/2020"
output:
  github_document:
    html_preview: false
...
# merge counts with barcode_runs to get more metadata on the samples, on library and sample
cols <- c('library', 'sample', 'sort_bin')
counts <- merge(counts, barcode_runs[, cols])
...
for(i in 1:nrow(barcode_runs)){
  ...
  bin <- as.character(barcode_runs$sort_bin[i])
  if(sum(counts[library==lib & sort_bin==bin,"count"]) < barcode_runs$number_cells[i]){ #if there are fewer reads from a sortseq bin than cells sorted
    counts[library==lib & sort_bin==bin, count.norm := as.numeric(count)] #don't normalize cell counts, make count.norm the same as count
    print(paste("reads < cells for",lib,bin,", un-normalized")) #print to console to inform of undersampled bins
  }else{
    ratio <- sum(counts[library==lib & sort_bin==bin,"count"])/barcode_runs$number_cells[i]
    counts[library==lib & sort_bin==bin, count.norm := as.numeric(count/ratio)] #normalize read counts by the average read:cell ratio, report in new "count.norm" column
  }
}
...
counts_lib1 <- dcast(counts[library=="lib1",], barcode + variant_call_support + target + variant_class + aa_substitutions + n_aa_substitutions + codon_substitutions + n_codon_substitutions ~ library + sort_bin, value.var="count.norm")
counts_lib2 <- dcast(counts[library=="lib2",], barcode + variant_call_support + target + variant_class + aa_substitutions + n_aa_substitutions + codon_substitutions + n_codon_substitutions ~ library + sort_bin, value.var="count.norm")
...
invisible(counts_lib1[,c("ML_meanF","ML_sdF") := tryCatch(calc.MLmean(b1=lib1_bin1*20,b2=lib1_bin2*20,
                                                                      b3=lib1_bin3*20,b4=lib1_bin4*20,
                                                                      min.b1=log(20),min.b2=log(947),min.b3=log(14416),
                                                                      min.b4=log(37796),max.b4=log(200000)),
                                                          error=function(e){return(list(as.numeric(NA),as.numeric(NA)))}),by=barcode])
counts_lib1[,total_count := sum(lib1_bin1,lib1_bin2,lib1_bin3,lib1_bin4),by=barcode]
...
invisible(counts_lib2[,c("ML_meanF","ML_sdF") := tryCatch(calc.MLmean(b1=lib2_bin1*20,b2=lib2_bin2*20,
                                                                      b3=lib2_bin3*20,b4=lib2_bin4*20,
                                                                      min.b1=log(20),min.b2=log(947),min.b3=log(13565),
                                                                      min.b4=log(35238),max.b4=log(200000)),
                                                          error=function(e){return(list(as.numeric(NA),as.numeric(NA)))}),by=barcode])
counts_lib2[,total_count := sum(lib2_bin1,lib2_bin2,lib2_bin3,lib2_bin4),by=barcode]
...
```{r violins_expression_distribution, echo=T, fig.width=9, fig.height=4.5, fig.align="center", dpi=300,dev="png"}
p1 <- ggplot(counts_filtered_lib1[!is.na(ML_meanF),],aes(x=variant_class,y=ML_meanF))+
...
p2 <- ggplot(counts_filtered_lib2[!is.na(ML_meanF),],aes(x=variant_class,y=ML_meanF))+
  geom_violin(scale="width")+stat_summary(fun.y=median,geom="point",size=1)+
...
rbind(counts_filtered_lib1[,.(library, barcode, variant_call_support, total_count, ML_meanF, delta_ML_meanF, var_ML_meanF, 
                           variant_class, aa_substitutions, n_aa_substitutions)],
      counts_filtered_lib2[,.(library, barcode, variant_call_support, total_count, ML_meanF, delta_ML_meanF, var_ML_meanF, 
                           variant_class, aa_substitutions, n_aa_substitutions)]
      ) %>%
